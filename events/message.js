...
module.exports = class {
    ...
    async run (message) {
        ...
        if(neededPermissions.length > 0) {
            return message.channel.send(`__**${this.client.config.emotes.error} Permissions manquantes**__\n\nJ'ai besoin des permissions suivantes pour le bon fonctionnement de cette commande : ${neededPermissions.map((p) => permissions[p]).join(", ")}`);
        }
        ...
        if(permLevel < this.client.levelCache[cmd.conf.permLevel]){
            return message.channel.send(this.client.emotes.error+" | Cette commande nécessite le niveau de permission : `"+cmd.conf.permLevel+"` !");
        }

        let time = cooldowns[`${message.author.id}${cmd.help.name}`] || 0;
        if(time > Date.now()){
            return message.channel.send(this.client.emotes.error+" | Vous devez attendre encore **"+Math.ceil((time - Date.now())/1000)+"** seconde(s) avant de pouvoir de nouveau effectuer cette commande !");
        } else {
            cooldowns[`${message.author.id}${cmd.help.name}`] = Date.now() - cmd.conf.cooldown;
        }
